    ...
    chrome.runtime.sendMessage({ action: "PAST_MESSAGES" }, (response) => {
      try {
        ...
        for (let i = 0; i < response.data.length; i++) {
          ...
          this.createMessage(response.data[i].role == "model" ? d(response.data[i].message) : response.data[i].message, response.data[i].role == "model" ? "assistant" : "user", response.data[i].time, response.data[i].id);
        }

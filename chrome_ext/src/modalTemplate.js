...
function initializeModal(modalOverlay) {
  ...
  // Add hover effects to section toggles
  ...
  // Add tooltips for helpful explanations
  addTooltips(modalOverlay);

  // Load saved preferences (in real extension, use chrome.storage.local)
...
async function handleGenerate(modalOverlay) {
  const generateBtn = modalOverlay.querySelector('#dtgGenerateBtn');
...
function showOutput(modalOverlay, content) {
  ...
  // Add hover effects to output toggle
...
function loadUserPreferences(modalOverlay) {
  try {
    // In real Chrome extension, replace with chrome.storage.local.get()
...
function saveUserPreferences(modalOverlay) {
  try {
    const formData = collectFormData(modalOverlay);
    // In real Chrome extension, replace with chrome.storage.local.set()
...
function addKeyboardNavigation(modalOverlay) {
  ...
  // Tab navigation for collapsible sections
...
function showValidationMessage(modalOverlay, message, type = 'error') {
  hideValidationMessage(modalOverlay); // Remove existing message
...
//       }
//     </style>
//     <h2 style="margin: 0 0 15px 0; color: #333;">Modal from Side Panel</h2>
//     <p style="margin: 0 0 20px 0; color: #666;">This modal was opened from the side panel!</p>

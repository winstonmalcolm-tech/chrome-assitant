class EmailGrammarChecker {
  ...
  findComposeArea() {
    ...
    if (this.currentProvider === "outlook") {
      const candidates = Array.from(document.querySelectorAll('div[contenteditable="true"]')).filter((el) => {
  ...
  isComposeBoxOpen() {
    ...
    if (this.currentProvider === "outlook") {
      ...
      if (!found) {
        const candidates = Array.from(document.querySelectorAll('div[contenteditable="true"]')).filter((el) => {
          const rect = el.getBoundingClientRect();
          return el.offsetParent !== null && rect.width > 200 && rect.height > 50 && el.closest('[data-app-section="ReadingPane"]');
        });
...
setTimeout(initEmailGenerator, 2e3);
setTimeout(initEmailGenerator, 5e3);

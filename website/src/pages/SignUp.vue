...
function isExtensionInstalled(timeout = 5000) {
  return new Promise((resolve) => {
    const listener = (event) => {
      if (event.data?.from === "alinea.ai_1289" && event.data?.status === "installed") {
    ...
    <div class="flex flex-col w-full items-center">
      <div className="mb-4 text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Join thousands of professionals using Tatalina AI
        </p>
      </div>

      <div
        class="w-[50%] py-10 px-5 border-[1px] border-gray-300 rounded-lg bg-[#F4F9FF]"
      >
        <div class="mb-4 text-center">
          <h1 class="text-xl text-foreground font-bold mb-2">Sign Up</h1>
          <p class="text-foreground">
            Create your account to get started with Tatalina AI
          </p>
        </div>
        ...
        <form @submit.prevent="handleSignUp()" class="flex flex-col items-center gap-4">
          <input
            type="text"
            v-model="data.name"
            class="focus:ring-2 focus:ring-blue-500/50 w-full p-2 border-[1px] border-gray-300 rounded-lg outline-none"
          ...
          <p>
            ...
            <router-link class="text-blue-400 hover:underline" to="/terms"
          ...
          <button
            v-else
            type="submit"
            class="px-4 py-2 bg-primary-500 text-white w-[40%] rounded-lg hover:bg-blue-500 transition duration-300 cursor-pointer"
          >
            Sign Up
          </button>
        </form>
